...
1. **Ausência de proposta aberta, intercultural e validada.** Não existe, na literatura científica e acadêmica, proposta de estrutura de dados **aberta, intercultural e validada** que represente com precisão e robustez a complexidade do conjunto **dado, informação e conhecimento** tradicional associado à biodiversidade, em suas diferentes manifestações, em diferentes culturas, línguas e povos — cada qual com sua visão de mundo única. O que a literatura documenta são implementações parciais ou restritas: (i) bases fechadas e voltadas à proteção defensiva contra patentes em sistemas médicos clássicos codificados, como a Traditional Knowledge Digital Library (TKDL) da Índia (CSIR, 2006); (ii) ontologias e grafos de conhecimento monoculturais e restritos a doutrinas médicas formais, como o Traditional Chinese Medicine Language System (TCMLS/GFO-TCM; ISO/TS 17938:2014; ZHOU et al., 2019) e vocabulários do Ayurveda; (iii) sistemas de gestão arquivística e atribuição de direitos digitais comunitários sem modelagem biológica ou relacional de espécies, órgãos e preparações (Mukurtu CMS; CHRISTEN, 2012; Local Contexts Hub; ANDERSON; CHRISTEN, 2019; MONTENEGRO, 2019); e (iv) extensões parciais de biodiversidade voltadas a ocorrência e categorias genéricas de uso utilitário (DwC-DP, SocioBio/SiBBr), cujos campos de política de uso limitam-se a direitos autorais ocidentais sem previsão de protocolos culturais. Nenhuma das estruturas existentes integra a distinção enunciativa entre a voz do detentor e o registro de terceiros à soberania sobre o nível de acesso (CARROLL et al., 2020; JENNINGS et al., 2023; ZANK et al., 2025).
...
**Científica.** Não há, no Brasil nem no cenário internacional, modelo de dados publicado que trate a distinção Conhecimento × Evidência como propriedade de primeira classe do registro, com consequência operacional sobre a rotulagem de acesso e controle soberano. Os padrões de referência (Darwin Core, DwC-DP, SocioBio/SiBBr) tratam de ocorrência e de uso utilitário (`usedTo`, `organismPart`), não de regime enunciativo ou autoria indígena. A tabela `usage-policy` do DwC-DP, verificada na especificação primária do GBIF/TDWG, restringe-se a campos patrimoniais de copyright e licenciamento ocidental (`rights`, `rightsHolder`, `owner`, `license`, `accessRights`), contendo **zero** suporte a protocolos culturais, sazonalidade, segredo sagrado ou governança de povos tradicionais. No âmbito dos registros globais, os relatórios da Organização Mundial da Propriedade Intelectual (WIPO/GRTKF/IC/46/8 e 46/12) reiteram que a definição de estruturas de dados padronizadas para conhecimento tradicional permanece como lacuna e "trabalho futuro". Kock et al. (2026), ao caracterizar riqueza de espécies arbóreas em Terras Indígenas na Bolívia com Red List, GBIF e BBON, mostram empiricamente o mesmo vazio no domínio de ocorrência biológica: a maioria dos dados vem de instituições internacionais sem qualquer metadado sobre contribuição indígena, o que "complicates sensitive interpretation and weakens visibility of Indigenous knowledge systems" — evidência externa e quantificada de que nenhum padrão vigente, nem para CTA nem para ocorrência, registra formalmente *quem fala*.
