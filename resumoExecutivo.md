...
A interlocução com o Ponto-Focal não é conversa avulsa: é um **ciclo**, e o ciclo é parte do método do projeto. Cada reunião gera um resumo versionado em [`docs/reunioes/`](docs/reunioes/), e cada resumo é confrontado com os documentos de arquitetura no log [`docs/reunioes/impactos-na-arquitetura.md`](docs/reunioes/impactos-na-arquitetura.md), que classifica cada decisão em *fecha*, *contradiz*, *acrescenta*, *confirma* ou *abre* e nomeia o documento que precisa mudar. É por esse canal que a visão de **uso, impacto e benefício** da arquitetura em campo entra no desenho — e o efeito é mensurável: das duas primeiras reuniões saíram quinze itens de impacto, cinco deles contradizendo texto normativo já escrito.
...
- **A federação não tem, hoje, nenhuma comunidade tradicional com voz direta.** O USEFLORA — cujo Comitê Gestor é misto, com academia e comunidades tradicionais — é a única iniciativa parceira com esse alcance, e designou seu Ponto-Focal em 17/09/2026 (Sofia Zank). Ainda assim, o USEFLORA serve como interlocução por procuração para pendências que nascem também de outras iniciativas (BioCultRelatos, Pluriverso) cujas comunidades afetadas ainda não estão na conversa — isso é insumo de desenho, nunca consentimento. As demais iniciativas parceiras seguem sem Ponto-Focal designado.
