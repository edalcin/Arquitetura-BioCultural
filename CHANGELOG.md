...
- **`resumoExecutivo.md`** na raiz — introdução objetiva, didática e sintética à proposta: o problema, a arquitetura federada com diagrama Mermaid da topologia, os dois eixos (fonte e regime enunciativo), o estado real de cada componente, a governança com a nota honesta de que o Comitê Federado não existe, o que a plataforma nunca fará, e o que ainda não está resolvido. É a porta de entrada para quem chega ao projeto sem leitura prévia
- **Verbete `Ponto-Focal` no glossário da federação** (`CONTEXT.md`, seção "Federação") — a pessoa que uma **iniciativa parceira designa** para a interlocução com a arquitetura. É **canal, nunca titular**: a resposta de um Ponto-Focal é a posição da iniciativa que o designou, e nunca substitui o consentimento da comunidade detentora sobre um registro concreto, que é coletivo (Lei nº 13.123/2015, art. 10, §1º) e se dá registro a registro. Quem indica é a iniciativa, nunca a arquitetura. `_Avoid_: Representante, Interlocutor, Mediador, Contato` — `representante` já significa, no repositório, quem senta no Comitê Federado por um membro
- **Nota de estado na `governanca/propostaGovernanca.md` §2** — as três camadas de governança são **proposta em consulta**, não estrutura existente; o **Comitê Federado não está constituído** (o bloqueador E4 de §6.1 é uma das razões); hoje o pesquisador proponente acumula as três camadas. O único mecanismo em operação real é o Ponto-Focal por iniciativa parceira, e ele **nasceu de baixo**, pela necessidade prática de resolver pendências, antes e independentemente da estrutura formal proposta
- **`propostaGovernanca.md` §5.12, item 9** — compromisso negativo novo: nunca fazer trafegar por provedor externo de IA conteúdo de registro cujo nível efetivo de acesso seja diferente de `public`. Alcança extração de metadados, sumarização, *embeddings* e busca conversacional, e vale para o conteúdo e para o prompt que o carrega. Torna verificável o item 3, que proíbe o treinamento mas não o **envio** — que é o que de fato acontece a cada chamada
- **`docs/reunioes/`** — pasta nova para memórias de reunião com iniciativas parceiras, e a primeira delas: **`reuniao-useflora-2026-08-18.md`**, versão publicável (sem atribuição nominal de falas, nomes apenas nos encaminhamentos com responsável, termos técnicos da transcrição automática corrigidos, nota de que é sumário e não ata aprovada)
- **Pauta 7 — o detentor apagado pela publicação**, com roteiro completo, em `docs/conhecimento/pauta-comunidades.md`: em fonte secundária, o detentor foi frequentemente registrado pelo autor como "informante, 62 anos" e é inalcançável. Classificar como Evidência do autor resolve *quem manda no registro*; não resolve que o conhecimento tem dono e o dono foi apagado. É a única pauta que o USEFLORA pode responder de imediato, e bloqueia dado já em produção no BioCultDB
- **Roteiro das pautas 5 (gravações e oficinas coletivas) e 6 (o sagrado)**, que estavam nomeadas sem roteiro desde a v3.9.0 e a v3.10.0
- **Nota de retificação no `ADR-011`** — a afirmação de que "não há demanda reportada por uso offline até o momento" deixou de ser verdadeira: a demanda foi levantada em 18/08/2026 (sincronização assíncrona por laptop ou pen-drive para comunidades com baixa conectividade). A nota **não** decide o mecanismo: o `ADR-005` fixou um SQLite com WAL por unidade, que é *single-writer*, e não há alternativas comparáveis ainda. Registra-se também que os diagramas C4 já traziam "offline-first para coleta" — a contradição era interna ao repositório
- **§10-bis do `docs/proximosPassos.md`** — a sessão de 2026-09-03, com seis pendências novas vindas da reunião: ⑬ indicação do ponto-focal do USEFLORA (em atraso), ⑭ princípios mínimos da federação, ⑮ proposta de governança operacional a receber, ⑯ sincronização assíncrona, ⑰ política de IA × dado sensível, ⑱ capacitação como repartição não monetária
- **§15 da caracterização** — o arquivo `conhecimentoPanara.mp4` (61,9 MB, não rastreado) passa a ser documentado: o que é, povo indicado no nome, detentor e consentimento `[não verificado]`, e o registro de que o destino correto é armazenamento soberano fora da árvore de qualquer repositório (pendência ⑩)

### Modificado

- **`conhecimento/` migrado para `docs/conhecimento/`.** Referências atualizadas em `README.md` (árvore da documentação e navegação), `docs/projetoPesquisa.md` §11, `ADR-015` (Referências) e `docs/proximosPassos.md`. As entradas da v3.7.0 deste changelog preservam os caminhos antigos por serem registro histórico
- **`sessao-2026-08-13-decisoes-e-pendencias.md` renomeado para `docs/conhecimento/pauta-comunidades.md`** e reescrito: deixa de ser registro de sessão — cujas camadas já estavam duplicadas em `docs/proximosPassos.md` §1, §2, §3, §10.1 e §11.2 — e passa a ser **o documento objetivo do Ponto-Focal**. A antiga §5 sobreviveu reorganizada em **duas seções estruturais**: *Pautas de desenho*, decidíveis com um corpo de representação mista e cada uma com estado em duas colunas (consideração do parceiro / validação com a comunidade afetada), e *Pautas de consentimento*, que só a comunidade detentora responde, registro a registro. A fronteira é estrutural, não uma nota de rodapé: fica visualmente impossível levar uma pauta de consentimento a uma reunião de comitê
- **Seção de iniciativas parceiras** no mesmo documento, com o estado do USEFLORA: ponto-focal **solicitado em 18/08/2026, prazo sugerido de duas semanas, em atraso**; quais pautas se aplicam a ele e quais não (as pautas 3, 4 e 5 nascem de gravação de campo e não são a pauta de quem trabalha com artigos já publicados); e o registro de que ele serve, informalmente, como interlocução **por procuração** para pendências do BioCultRelatos e do Pluriverso. Nas pautas de consentimento, o campo "com quem" fica **explicitamente vazio** para essas duas unidades — mostrar o vazio é o ponto
- **`docs/proximosPassos.md`** consolidado como **o único documento de pendências**: §5 passa a apontar para a pauta e lista sete pautas; §6 acrescenta a auditoria de prompts do BioCultDB; §8 remapeia os caminhos; §11.2 recebe as próximas ações
- **`BioCultDB/docs/proximosPassos.md`** (outro repositório) — pendência nova de auditoria dos prompts de extração por IA contra vazamento de dado sensível, explicitamente distinta da pendência de acurácia já registrada lá

### Contexto da Versão

Nenhuma decisão de modelo de dados. A versão faz duas coisas: consolida os documentos de pendência em um só, e dá registro ao mecanismo pelo qual a arquitetura de fato evolui — a interlocução com iniciativas parceiras. Não houve ADR nova: o papel do Ponto-Focal não resultou de alternativa descartada, é a prática que emergiu quando ficou claro que há pendências que a arquitetura não pode resolver sozinha. Registrar isso é, ele mesmo, resultado de pesquisa (`docs/projetoPesquisa.md` §7.2, procedimento 6).
...
- **`same as` estava errado no vínculo entre Relato e item de acervo.** O Relato **não é** a exsicata do herbário; ele fala sobre ela. `same as` afirma identidade e colapsaria os dois no índice do Pluriverso, apagando exatamente a distinção que a ADR-015 existe para preservar. O caso normal passa a ser `refers to`
- **ADR-015, §"O que esta ADR não decide"**: Q1 (regime no glossário da federação) constava como aberta desde a v3.7.0, que já a respondeu. Fechada. A contagem passa de seis para **quatro questões abertas** (Q3 em diante), refletida no Status e na Data de Revisão
...
O sintoma que a expôs é estrutural e verificável: o registro em primeira pessoa, enunciado por quem detém o conhecimento — na língua da comunidade, em campo, junto à planta —, não tinha onde existir no modelo de dados. `ADR-003:337-338` é `media: []` com o comentário `// (futuro)`, sem nenhum campo de nível de acesso, consentimento ou detentor associado a uma mídia primária. O modelo formalizado protegia evidência de terceiros; o registro em primeira pessoa ficava de fora.
...
Duas retificações menores ficam registradas na própria ADR-015, sem reescrever o ADR-003, que segue em status *Proposto* e entra na mesma rodada de validação com comunidades: `metadata.language: "pt-BR"` migra para ISO 639-3 (K5) — `pt-BR` não codifica língua indígena sem código ISO 639-1, e o BioCultTermos já exige ISO 639-3 desde a migração de 2601 conceitos de `pt` para `por` —, e `media` deixa de ser "(futuro)".

Pendências abertas e nomeadas: as seis questões de decisão da ADR-015; e nenhuma unidade implementou ainda os campos de acesso do ADR-003.
